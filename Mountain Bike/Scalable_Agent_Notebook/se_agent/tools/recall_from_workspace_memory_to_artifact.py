...
__all__ = ["RecallFromWorkspaceMemoryToArtifactTool"]

@register_tool
class RecallFromWorkspaceMemoryToArtifactTool(TransformTool):
    """
    Creates a registry artifact from a workspace memory entry, enabling tools to consume it.
    """
    TOOL_NAME = "recall_from_workspace_memory_to_artifact"
    DESCRIPTION = "MATERIALIZE A WORKSPACE MEMORY ENTRY AS A TYPED ARTIFACT (FOR TOOL USE)."
    CATEGORY = "transform"
    ...
    name = TOOL_NAME
    description = "Recall a workspace value by name and write it as an artifact."
    ...
    def run(self, input_data: Dict[str, Any], artifacts: ArtifactRegistry, package_name: Optional[str] = None, **_: Any) -> Dict[str, Any]:
        ...
        meta = {"ui_summary": f"Materialized from workspace memory '{entry_name}'", "name": persist_name}
