...
class FormatYAMLReportTool(DisplayTool):
    ...
    Inputs:
      • yaml_content_name / yaml_content_id           : identifies a 'yaml_content' artifact (REQUIRED)
      • prompt_name / prompt_id : identifies a 'prompt' artifact with text (OPTIONAL)
    ...
    description = (
        ...
        "Required: yaml_content_name or yaml_content_id of a 'yaml_content' artifact. "
        "Required: prompt_name or prompt_id of a 'prompt' artifact (string) to steer formatting. "
        "Planner rules: (1) Always pass the YAML by name/id; (1) Always pass the prompt by name/id;"
        "(3) No raw YAML in inputs. Example: "
        "{\"actions\":[{\"tool\":\"format_yaml_report\",\"input\":{"
        "\"yaml_content_name\":\"Bike_CS_yaml\",\"prompt_name\":\"context_view_prompt\"}}]}"
    ...
    def render(self, input_data, artifacts, package_name=None):
        ...
        yaml_name = input_data.get("yaml_content_name")
        ...
        if yaml_name:
            yaml_art = self._get_by_name(artifacts, pkg_name, yaml_name)
        elif yaml_id:
            yaml_art = self._get_by_id(artifacts, pkg_name, yaml_id)

        if not yaml_art or not isinstance(yaml_art.content, str):
            return "<p style='color:red'>❌ Provide name/id of a 'yaml_content' artifact with string content.</p>"

        yaml_text = yaml_art.content

        # --- Resolve optional prompt artifact
        prompt_text = None
        p_name = input_data.get("prompt_name")
        p_id    = input_data.get("prompt_id")

        if p_name:
            p_art = self._get_by_name(artifacts, pkg_name, p_name)
