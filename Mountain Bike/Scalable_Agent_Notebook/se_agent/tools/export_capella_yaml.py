...
class ExportCapellaYAMLTool(TransformTool):
    ...
    Preferred inputs:
      ...
      • model_path_name  : name of artifact with content=str (.aird path), e.g., "Bike_Path"
      • resources_name   : name of artifact with content=dict (capellambse resources), e.g., "Bike_Resources"

    Alternatives (if no selection_name):
      • uuids             : list[str] of UUIDs to export
      • uuid              : single UUID string

    Optional:
      • name             : name to assign to the created YAML artifact (e.g., "MB_yaml")

    Behavior:
      1) Builds a capellambse.MelodyModel from provided names.
    ...
    description = (
        ...
        "Preferred inputs: selection_name (capella_selection), model_path_name (e.g., 'Bike_Path'), "
        "resources_name (e.g., 'Bike_Resources'). Alternatively provide uuids=[...] or uuid='...'. "
    )
    artifact_type = "yaml_content"

    # minimal name lookup consistent with your registry (scan package)
    ...
    def transform(self, input_data, artifacts, package_name=None):
        ...
        if selection_name and artifacts:
            ...
            if not sel_art or not isinstance(sel_art.content, list):
                raise ValueError(f"Selection name '{selection_name}' not found or content is not a list.")
        ...
        # ---- Resolve model path / resources via names
        ...
        if not (model_path_name and resources_name):
            raise ValueError("Provide model_path_name and resources_name.")
        ...
        if not art_path or not isinstance(art_path.content, str):
            raise ValueError(f"Artifact '{model_path_name}' not found or content is not a string path.")
        if not art_res or not isinstance(art_res.content, dict):
            raise ValueError(f"Artifact '{resources_name}' not found or content is not a dict.")
        ...
        # Optional: give the YAML artifact a default name if none provided
