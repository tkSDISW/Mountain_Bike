    ...
    def run(self, input_data, artifacts, package_name=None, **kwargs):
        ...
        if artifacts and pkg_name:
            ...
            art = artifacts.add_artifact(
                pkg_name, type_=artifact_type, content=new_content, metadata=metadata or {}
            )

            # 👇 NEW: optional alias support
            alias = (input_data or {}).get("alias") or (metadata or {}).get("alias")
            alias_msg = None
            if alias:
                try:
                    aliased = artifacts.alias_artifact(pkg_name, artifact_type, alias)
                    alias_msg = getattr(aliased, "_announce", None)
                except Exception as e:
                    alias_msg = f"⚠️ Failed to assign alias '{alias}': {e}"

            # return both messages; agent will show both after the HTML patch above
            combined_msg = msg  # your concise summary
            return {
                "message": combined_msg,
                "artifact_message": getattr(art, "_announce", None) if not alias_msg else alias_msg,
                "artifact_id": art.id,
                "artifact_type": art.type,
                "package_name": pkg_name,
