...
class AgentCore:
    ...
    def interactive_chat(self, package_name=None, context="You are a helpful assistant."):
        ...
        # =============================================================
        # 1) Chat history output (this replaces old `output_area`)
        # =============================================================
        ...
        # =============================================================
        # 2) Larger input + wrapper with border (avoid spillover)
        # =============================================================
        user_input = widgets.Textarea(
            placeholder="Type your prompt (Enter or click Send)…",
            rows=10,
            layout=widgets.Layout(
                width="100%",
                height="140px",
        ...
        send_button = widgets.Button(
            ...
            tooltip="Execute your prompt",
            layout=widgets.Layout(width="140px", height="44px", margin="10px 0 0 0"),
        )
        exit_button = widgets.Button(
            description="Exit",
            tooltip="Close interactive chat",
            layout=widgets.Layout(width="120px", height="44px", margin="10px 0 0 10px"),
        )
        ...
        def _history_key_handler(widget, content, buffers):
            nonlocal history_index
            event = content.get("event") if isinstance(content, dict) else None
            if not event or event.get("type") != "keydown":
                return
            key = event.get("key")
            if key == "ArrowUp" and command_history:
                history_index = max(0, history_index - 1)
                user_input.value = command_history[history_index]
            elif key == "ArrowDown" and command_history:
        ...
        def _submit_on_ctrl_enter(widget, content, buffers):
            event = content.get("event") if isinstance(content, dict) else None
            if not event or event.get("type") != "keydown":
                return
            if event.get("key") == "Enter" and (event.get("ctrlKey") or event.get("metaKey")):
                send_button.click()      

        # Attach once (outside send_message)
        user_input.on_msg(_history_key_handler)
        user_input.on_msg(_submit_on_ctrl_enter)
        def send_message(_):
            ...
            # store in history BEFORE clearing input
        ...
        controls = widgets.HBox([
            input_container,
            widgets.VBox([send_button, exit_button], layout=widgets.Layout(align_items="stretch"))
        ])
        ...
        display(chat_history, ui_box, widgets.HBox([send_button, exit_button]), file_dropdown)
